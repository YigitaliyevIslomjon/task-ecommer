...
import { CreateForm } from '@/modules/post/forms';
...
const CreateModal: React.FC<IProps> = ({ onClose, isOpen }) => {
  return (
    <>
      <Modal width={1000} title="Create Post" open={isOpen} onOk={onClose} onCancel={onClose} footer={false}>
        <>
          <Spacer size={24} />
          <CreateForm
            onSuccess={() => {
              message.success('Post muvoffaqiyatli yaratildi');
              onClose();
            }}
          >
            {({ formState }) => {
