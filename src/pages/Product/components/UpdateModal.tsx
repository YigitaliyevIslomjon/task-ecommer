...
import { UpdateForm } from '@/modules/product/forms';
import useSingle from '@/modules/product/hooks/useSingle';
...
const UpdateModal: React.FC<IProps> = ({ onClose, isOpen, id }) => {
  ...
  return (
    <>
      <Modal width={1000} title="update product" open={isOpen} onOk={onClose} onCancel={onClose} footer={false}>
        {!isFetched ? (
          ...
          <>
            ...
            <UpdateForm
              ...
              values={item}
              onSuccess={() => {
                message.success('product muvoffaqiyatli updated');
                onClose();
              }}
            >
              {({ formState }) => {
